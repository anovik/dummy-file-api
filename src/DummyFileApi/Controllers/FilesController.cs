    ...
    GenerationRateLimiter rateLimiter) : ControllerBase
{
    ...
    /// <param name="type">One of the types returned by <c>GET /api/files/types</c> (e.g. <c>txt</c>, <c>csv</c>, <c>pdf</c>, <c>jpeg</c>, <c>png</c>, <c>zip</c>).</param>
    ...
    /// (omit for the first palette color); <c>zip</c> uses it to pick the entry's filler phrase from a
