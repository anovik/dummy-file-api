    ...
    GenerationRateLimiter rateLimiter) : ControllerBase
{
    ...
    /// <param name="type">One of the types returned by <c>GET /api/files/types</c> (e.g. <c>txt</c>, <c>csv</c>, <c>pdf</c>, <c>jpeg</c>, <c>png</c>, <c>zip</c>, <c>docx</c>, <c>xlsx</c>, <c>json</c>, <c>tar</c>).</param>
    ...
    /// (omit for the first palette color); <c>zip</c>/<c>docx</c>/<c>tar</c> use it to pick the filler phrase from a
    ...
    /// value, for 1; also falls back to 1 if the requested size is too small to fit even one row at that Id);
    /// <c>txt</c> ignores it entirely.
    /// </param>
