    ...
    GenerationRateLimiter rateLimiter) : ControllerBase
{
    ...
    /// <param name="type">One of the types returned by <c>GET /api/files/types</c> (e.g. <c>txt</c>, <c>csv</c>, <c>pdf</c>, <c>jpeg</c>, <c>png</c>, <c>zip</c>, <c>docx</c>, <c>xlsx</c>).</param>
    ...
    /// fixed set (omit for the first), which changes the bytes but not the size; <c>csv</c> and <c>xlsx</c>
    /// use it as the starting row Id, rows counting up from there (omit, or pass a non-positive value, for 1;
    /// also falls back to 1 if the requested size is too small to fit even one row at that Id); <c>txt</c>
    /// ignores it entirely.
    /// </param>
